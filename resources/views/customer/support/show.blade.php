...
<div class="max-w-3xl">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl border p-5 {{ $message->is_admin_reply ? 'border-l-4 border-l-amber-500' : '' }}">
            <div class="flex items-start gap-3">
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <a href="{{ $message->attachment_url }}" target="_blank" class="inline-flex items-center gap-1 mt-2 text-sm text-amber-600 hover:text-amber-700">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl border p-6">
        ...
        <form method="POST" action="{{ route('customer.support.reply', $ticket) }}" enctype="multipart/form-data">
            @csrf
            <textarea name="message" rows="4" required placeholder="Tulis balasan Anda..." class="w-full px-4 py-3 bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-amber-500 mb-4"></textarea>
            <div class="flex items-center justify-between">
                ...
                <button type="submit" class="px-6 py-2.5 bg-amber-600 text-white font-semibold rounded-xl hover:bg-amber-700 transition">Kirim Balasan</button>
            </div>
        </form>
    </div>
    @endif
