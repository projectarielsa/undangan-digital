...
<div class="max-w-lg mx-auto">
    <div class="bg-white dark:bg-gray-800 rounded-2xl border p-8 text-center">
        ...
        <p class="text-3xl font-bold text-amber-600 mb-2">Rp {{ number_format($payment->total_amount, 0, ',', '.') }}</p>
        <p class="text-sm text-gray-500 mb-8">Order ID: {{ $payment->order_id }}</p>
        <button id="pay-button" class="w-full py-3 bg-gradient-to-r from-amber-600 to-amber-700 text-white font-semibold rounded-xl shadow-lg shadow-amber-500/25 hover:shadow-amber-500/40 transition-all transform hover:scale-[1.02]">Bayar Sekarang</button>
