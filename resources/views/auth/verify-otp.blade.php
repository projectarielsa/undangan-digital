...
<div class="text-center" x-data="otpForm()">
        ...
        <div class="flex justify-center gap-3">
            <template x-for="(digit, index) in otp" :key="index">
                <input type="text" maxlength="1" x-model="otp[index]" @input="handleInput($event, index)" @keydown.backspace="handleBackspace($event, index)" @paste="handlePaste($event)"
                    class="w-12 h-14 text-center text-xl font-bold bg-gray-50 dark:bg-gray-700 border-2 border-gray-200 dark:border-gray-600 rounded-2xl text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200" inputmode="numeric" pattern="[0-9]">
            </template>
        </div>
        <button type="submit" :disabled="otp.join('').length < 6" class="w-full py-3 px-4 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white font-semibold rounded-2xl shadow-lg shadow-blue-500/25 transition-all duration-200 transform hover:scale-[1.02] disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none">Verifikasi</button>
    </form>
    ...
    <div class="mt-6" x-data="countdown({{ $cooldown }})">
        <form method="POST" action="{{ route('verification.otp.resend') }}">
            ...
            <button type="submit" :disabled="remaining > 0" class="text-sm text-blue-600 hover:text-blue-700 font-semibold disabled:text-gray-400 disabled:cursor-not-allowed">
